...
describe('npm2yarn', () => {
	const commandPairs: [npm: string, yarn: string][] = [
		...
		// Incorrect, when https://github.com/armano2/npm-to-yarn/issues/7 is fixed this will make the test automatically fail
	...
	test.each(commandPairs)('GIVEN %s THEN transforms to %s', (npmCommand, yarnCommand) => {
		expect(npmToYarn(npmCommand, 'yarn')).toEqual(yarnCommand);
	});
});
